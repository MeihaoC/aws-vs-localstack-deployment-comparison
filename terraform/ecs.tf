...
resource "aws_ecs_task_definition" "order_api" {
  family                   = "final-mastery-order-api"
  network_mode             = "awsvpc"
...
resource "aws_ecs_task_definition" "order_processor" {
  family                   = "final-mastery-order-processor"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = "256"
  memory                   = "512"
...
resource "aws_ecs_service" "order_processor" {
  name            = "final-mastery-order-processor"
  cluster         = aws_ecs_cluster.main.id
